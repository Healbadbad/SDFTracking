...
from dataset import load_npz_rgbd_dataset
import numpy as np
...
def train_sdf(data_name, out_name):
    ...
    train_dataset = load_npz_rgbd_dataset(data_name)
    ...
    imageio.mimsave(f'images/{out_name}_pretraining_progress.gif', [np.uint8(im * 255) for im in test_images])
    ...
    training_cfg.render_interval = 10
    ...
    for epoch in range(0, num_epochs):
        ...
        for iteration, batch in t_range:
            # print("batch:", batch)
            image, depth, camera, camera_idx = batch[0].values()
            ...
            # imageio.mimsave(f"images/{out_name}_mask_gt.gif", [np.uint8(mask.cpu().numpy() * 255)])
            #imageio.mimsave(f"images/{out_name}_gt_depth.gif", [np.uint8(depth.cpu().numpy() * 255)])
            # imageio.mimsave(f"images/{out_name}_gt_depth.gif", [depth.cpu().numpy()])
            ...
            print("ray_bundle shape", ray_bundle.shape)
        ...
        if (
            ...
            imageio.mimsave(f'images/{out_name}_pointcloud.gif', [np.uint8(im * 255) for im in pointcloud_images])

            # Save a debug RGB image
            imageio.mimsave(f"images/{out_name}_rgb_gt.gif", [np.uint8(image[0].cpu().numpy() * 255)])
            test_images = render_images(
                sdf_render_model, create_surround_cameras(4.0, n_poses=20, up=(0.0, 1.0, 0.0), focal_length=2.0),
                image_size, file_prefix='volsdf'
            )
            imageio.mimsave(f'images/{out_name}_training_progress.gif', [np.uint8(im * 255) for im in test_images])
            ...
            try:
                ...
                imageio.mimsave(f'images/{out_name}_training_geometry.gif', [np.uint8(im * 255) for im in test_images])
