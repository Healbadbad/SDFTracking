# Loads an obj and uses an SDF to estimate its pose
...
def estimate_pose(obj_path, pose_sdf, image_size=256, n_images = 60, vis=False, device=None):
    ...
    # start_pos = np.array([-2, -2, 2])
    # end_pos = np.array([2, 2, 1.5])
    start_pos = np.array([0, 0, 2])
    end_pos = np.array([0, 0, 2])

    # Keep in center of image
    # start_pos = np.array([0, 0, 2])
    # end_pos = np.array([0, 0, 2])

    start_rot = np.array([0, 0, 0])
    end_rot = np.array([0, 0, 1*np.pi])
    # end_rot = np.array([0, 0, 1])
    # end_rot = np.array([0, 0, 0])
    ...
    for i in range(n_frames):
        ...
        rotation = torch.tensor(start_rot * (1 - i / n_frames) + end_rot * (i / n_frames)).to(device).float()
        R = pytorch3d.transforms.euler_angles_to_matrix(rotation, "XYZ")
        # TODO: Rotate correctly
        rotato = pytorch3d.transforms.Rotate(R)
        rotated_verts = rotato.transform_points(original_mesh.verts_list()[0])
        new_mesh = Meshes(verts=[rotated_verts], faces=[original_mesh.faces_list()[0]]).to(device) 
        new_mesh.textures = original_mesh.textures
        translated_mesh = new_mesh.offset_verts(offset)
        # rotated_mesh = translated_mesh.rotate(R)
        # translated_mesh[0].verts = rotato.transform_points(translated_mesh[0].verts)
        # translated_mesh = rotated_mesh
        # Prepare the camera:
        ...
        # TODO: Try optimizing rotation and translation separately
        t_optimizer = torch.optim.Adam(
            pose_sdf.translation_parameters(),
            lr=lr,
        )
        r_optimizer = torch.optim.Adam(
            pose_sdf.rotation_parameters(),
            lr=1e-3,
        )
